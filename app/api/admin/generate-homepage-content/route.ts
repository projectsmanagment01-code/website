...
} from "@/lib/ai-generation/homepage";
import type { SiteContext } from "@/lib/ai-generation/types";

interface GenerateRequest {
  field: 'heroTitle' | 'heroDescription' | 'metaTitle' | 'metaDescription';
  context: SiteContext;
...
export async function POST(request: NextRequest) {
  try {
    ...
    switch (field) {
      case 'heroTitle':
        result = await generateHeroTitle(context, apiKey, provider);
        break;
      case 'heroDescription':
        result = await generateHeroDescription(context, apiKey, provider);
        break;
      case 'metaTitle':
        result = await generateMetaTitle(context, apiKey, provider);
        break;
